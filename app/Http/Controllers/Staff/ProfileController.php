...
class ProfileController extends Controller
{
    public function getInfo_Account()
    {
        ...
        return view('staff.profile.info_account', compact('account'));
    }
    public function getUpdate_Account($id)
    {
        $account=accounts::Find($id);
        return view('staff.profile.upload_account',compact('account'));
    }
    public function postUpdate_Account($id,Request $req)
    {
         ...
         if($req->hasFile('image'))
            {
                ...
                {
                    return redirect('manage/profile/upload_account',$accounts->id)->with('notification','Bạn chỉ được chọn file có đuôi jpg,png,jpeg');
                }
        ...
        return redirect('manage/profile/info_account')->with('notification','Đã cập nhật thông tin tài khoản thành công');
    }
    public function getEdit_Password($id)
    {
       
            $account=accounts::Find($id);
            return view("staff.profile.edit_password",compact('account'));
    ...
    public function postEdit_Password($id,Request $req)
    {
        ...
        if(Auth::guard('account')->check())
        {
            ...
            if(Auth::guard('account')->attempt($credentials))
            {
               ...
               $account->save();
               return redirect('manage/profile/edit_password/'.$id)->with('notification','Đổi mật khẩu thành công');
            }
            else
            {
                return redirect('manage/profile/edit_password/'.$id)->with('notification','Mật khẩu cũ không đúng');
            }
        }
        else
        {
